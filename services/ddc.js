...
function getDisplayBrightness(bus) {
    ...
    const result = MyShell.exec(`xrandr --verbose | awk '/Brightness/ { print $2; exit }'`);


    var val = getValueFromString(result);
    Log.Log.log(`getDisplayBrightness - result: ${result}, converted: ${val}`);
...
function setDisplayBrightness(bus, value) {
    ...
    const result = MyShell.exec(`xrandr --output DP-4.1.8 --brightness ${convertedValue} && xrandr --output DP-4.8 --brightness ${convertedValue} && xrandr --output DP-4.1.1 --brightness ${convertedValue}`);
